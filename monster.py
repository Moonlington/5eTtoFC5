...
def parseMonster(m, compendium, args):
    ...
    #            m['name'] = "{} (DUPLICATE IN {})".format(m['name'],m['source'])
        ...
        if "special" in m['hp']:
            ...
            hp.text = "{}{}".format(m['hp']['average'], " ({})".format(m['hp']['formula'].replace(' ','')) if 'formula' in m['hp'] else "")
        ...
        if 'source' in m:
            name = m["name"]
            if args.addimgs and os.path.isdir("img") and not os.path.isfile(os.path.join(args.tempdir,"monsters", name + ".png")) and not os.path.isfile(os.path.join(args.tempdir,"monsters",name+".jpg")):
                ...
                if artworkpath is not None:
                    ...
                    copyfile(artworkpath, os.path.join(args.tempdir,"monsters",name + ext))
                    imagetag = ET.SubElement(monster, 'image')
                    imagetag.text = name + ext
            elif args.addimgs and os.path.isfile(os.path.join(args.tempdir,"monsters", name + ".png")):
                imagetag = ET.SubElement(monster, 'image')
                imagetag.text = name + ".png"
            elif args.addimgs and os.path.isfile(os.path.join(args.tempdir,"monsters", name + ".jpg")):
                imagetag = ET.SubElement(monster, 'image')
                        ...
                        if "colLabels" in e:
                            ...
                            for sube in e["items"]:
                                if type(sube) == str:
                                    ...
                                    subentries.append(utils.fixTags(sube["text"],m,args.nohtml))
                            text.text = "\n".join(subentries)
                        elif "type" in e and e["type"] == "spellcasting":
        ...
        if 'legendaryGroup' in m:
            ...
            for l in meta['legendaryGroup']:
                mname = m['legendaryGroup']['name'] if 'name' in m['legendaryGroup'] else m['legendaryGroup']
                if l['name'] != mname:
                    continue
                if 'lairActions' in l:
                    legendary = ET.SubElement(monster, 'legendary')
        ...
        print("Error while parsing",m['name'])